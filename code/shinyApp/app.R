...
funFacts <- readr::read_csv("C:/Users/Emily/OneDrive/Documents/GitHub/24-fall-EFUGH/Fugh_Capstone/data/funfacts.csv")

ui <- ui <- navbarPage(
  title="Carbon Capture Dashboard",  # The title of the navbar,
  tabPanel("Overview", uiOutput("introUI")),
  tabPanel("Fun Facts", uiOutput("funUI")),
...
server <- function(input, output, session) {
  ...
  output$introUI <- renderUI({
      navset_card_underline(
        nav_panel("Quick Overview", uiOutput("quick")),
        # Panel with plot ----
        nav_panel("Topic Introduction", includeHTML("introductionText.html")),
  ...
  output$quick <- renderUI ({
    
  })
  
  fact <- reactive({
    req(funFacts)
    random_numbers <- sample(1:nrow(funFacts), 3)
    
    funFacts %>% 
      filter(Number %in% random_numbers)
  })
  
  output$funUI <- renderUI({
    req(fact())
    page_fillable(
      layout_columns(
        theme = bs_theme(version = 5, bootswatch = "flatly"), # Apply a theme
          card(
            card_header("Fun Fact 1"),
            card_body(uiOutput("fact1"))
          ),
          card(
            card_header("Fun Fact 2"),
            card_body(uiOutput("fact2"))
          ),
          card(
            card_header("Fun Fact 3"),
            card_body(uiOutput("fact3"))
        )
      )
    )
  ...
  output$fact1 <- renderText({
    req(fact())
    fact()$Text[1]
  })
  
  output$fact2 <- renderText({
    req(fact())
    fact()$Text[2]
  })
  
  output$fact3 <- renderText({
    req(fact())
    fact()$Text[3]
  })
